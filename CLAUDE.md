...
- **Server → client function-prop boundary is real.** A server component CANNOT pass any function prop to a client component — module-level, async, "no React state," none of it matters. The ONLY function shape that crosses the boundary is a Server Action (a function in a file or block marked with `"use server"`). If a server component needs to provide default behavior for a client-component prop that's typed as a function, extract a thin `"use client"` wrapper that defines the default internally and only forwards consumer-passed overrides. Example: `src/components/storefront/_footer-newsletter.tsx` wraps `NewsletterSignup` so `Footer.tsx` can stay server-rendered while still providing a default `onSubscribe` stub. Underscore-prefix the wrapper file name to signal "internal helper, not part of the public API of this folder."
